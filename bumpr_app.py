    ...
    def generate_safely(self, prompt: str):
        ...
        if E_score >= self.threshold:
            ...
            if self.recohere_enabled:
                ...
                # Bumpr "fixes" the prompt and tries again
...
def create_gauge_chart(e_score, threshold):
    fig = go.Figure(go.Indicator(
        mode = "gauge+number",
        value = e_score,
        title = {'text': "<b>Bumpr Instability Score</b>", 'font': {'size': 18}},
        domain = {'x': [0, 1], 'y': [0, 1]},
        number = {'font': {'size': 28}},
        gauge = {
...
st.set_page_config(page_title="Bumpr Demo", layout="centered")
st.title("🛡️ Bumpr")
st.write("Automated AI Safety & Hallucination Control")

# Init session state
if "bumpr" not in st.session_state:
    st.session_state.bumpr = BumprEngine()
...
# --- SIDEBAR SETTINGS ---
with st.sidebar:
    st.header("⚙️ Bumpr Settings")
    
    # Threshold Slider
    new_threshold = st.slider(
        "Safety Sensitivity (Threshold)", 
        0.1, 1.0, 0.6, 0.05,
        help="If the Instability Score passes this line, Bumpr intervenes."
    )
    st.session_state.bumpr.threshold = new_threshold
    
    # Toggle Recoherence
    reco_on = st.toggle("Enable Auto-Correction", value=True)
    st.session_state.bumpr.recohere_enabled = reco_on
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        ...
        if "details" in msg:
            with st.expander("View Bumpr Inspection Log"):
                st.json(msg["details"])
...
if prompt := st.chat_input("Try typing: 'What is the capital of France?' or 'This is urgent!'"):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Bumpr is analyzing..."):
            
            # RUN BUMPR
            result = st.session_state.bumpr.generate_safely(prompt)
...
fig = create_gauge_chart(val, st.session_state.bumpr.threshold)
